...
export function RevenueOverview({ report: r }: { report: RevenueReport }) {
  ...
  return (
    <section className="mt-8 space-y-5" aria-labelledby="revenue-title">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl bg-emerald-950 p-6 text-white">
          <p className="text-sm text-emerald-100">Tổng giá trị đã thanh toán</p><p className="mt-3 break-words text-3xl font-black tabular-nums">{money(r.total)}</p>
          <p className="mt-3 text-sm text-emerald-100">{change === null ? "Chưa có giá trị ở kỳ trước để tính tăng trưởng" : `${change >= 0 ? "+" : ""}${change.toLocaleString("vi-VN", { maximumFractionDigits: 1 })}% so với ${r.days} ngày trước`}</p>
          <p className="mt-2 text-xs text-emerald-200">Kỳ trước: {money(r.previous)}</p>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[2fr_1fr]">
        <div className="min-w-0 rounded-2xl border border-zinc-200 bg-white p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-xs text-zinc-600"><span>● COD (xanh lá)</span><span>● Chuyển khoản / demo (tím)</span><span>Đỉnh: {money(peak === 1 && r.total === 0 ? 0 : peak)}</span></div>
          {r.paidCount === 0 ? <div className="flex h-56 items-center justify-center text-center text-sm text-zinc-500">Chưa có đơn đã thanh toán trong khoảng thời gian này.</div> : <div className="mt-5 overflow-x-auto pb-2">
            <div className="flex h-56 items-end gap-1 border-b border-zinc-200" style={{ minWidth: r.days > 30 ? 720 : 280 }} role="img" aria-label="Biểu đồ cột theo ngày. Số liệu đầy đủ trong bảng bên dưới.">
              {r.daily.map((row, index) => <div key={index} className="flex h-full min-w-0 flex-1 flex-col justify-end" title={`${row.date}: COD ${money(row.cod)}, chuyển khoản/demo ${money(row.transfer)}`}>
                <div className="rounded-t bg-violet-500" style={{ height: `${row.transfer / peak * 100}%` }} /><div className="bg-emerald-600" style={{ height: `${row.cod / peak * 100}%` }} />
              </div>)}
            </div>
